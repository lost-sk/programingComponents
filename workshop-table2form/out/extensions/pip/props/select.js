  ...
  render() {
    ...
    return (
      <div
        ...
        style={{backgroundColor:'rgb(235,235,235)',display:'flex',alignItems:'center',marginBottom:'5px'}}
      >
        {(prefix && !styleless) && (<span className="prefix-title" style={styleObj} title={this.intlFormat(prefix)}>{this.intlFormat(prefix)}</span>)}
        <div style={{display:'table-cell',verticalAlign:'middle',padding:'0 3px',color:'rgb(153, 165, 173)'}}>
          {this.state.title}
        </div>
        <Select
          ...
          value={this.isEmpty(this.state.value) ? this.state.defaultValue : this.state.value}
          mode={mode}
        ...
        >
          {
            this.state.options.map(({ key, value: $key, label, disabled: $disabled, style: $style }) => {
              return (
                <Option
