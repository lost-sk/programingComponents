  ...
  function CustomComp(props) {
    ...
    _this.getValue = function () {
      var _this$state = _this.state,
          ...
          bubbleVal = _this$state.bubbleVal;
      var data = {};
  ...
  _createClass(CustomComp, [{
    ...
    value: function componentDidMount() {
      ...
      if (this.props.isDesign) {
        this.setState({
          inputVal: mock.text,
          bubbleVal: mock.title
        });
        return;
      }

      this.runCode('componentDidMount', '内容加载事件脚本错误,请打开控制台查看错误信息');
      var config = this.state.config; // console.log(config, 'textpro config');

      this.getData();
      this.setState({
        inputVal: _.get(config, 'inputVal.value'),
        bubbleVal: _.get(config, 'bubbleVal.value'),
    ...
    value: function render() {
      var _this$state2 = this.state,
          ...
          bubbleVal = _this$state2.bubbleVal,
          autoFlag = _this$state2.autoFlag,
          defFlag = _this$state2.defFlag,
          position = _this$state2.position;
      var config = this.props.data._attrObject.data || {};
      var fontSize = _.get(config, 'fontSize.value', '13') + 'px';

      var fontColor = _.get(config, 'fontColor.color', '#000');
