...
class CustomComp extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      zoom: config.zoom != undefined ? config.zoom : '1',
  ...
  componentDidMount() {
    ...
    const chartOption = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          const { data } = params
          if (data && data.value && !Number.isNaN(data.value)) {
      ...
      cb: (res) => {
        if (attr === 'dataList') {
          const finalList = mapData.map((m) => {
            const findV = res.data.list.find((f) => m.name.includes(f['system.dim_0']))
            if (findV) {
