...
class CustomComp extends Component {
  ...
  componentDidMount() {
    ...
    const debounceFunc = debounce((zoom, cityMap, series) => {
      ...
      const { showCitizenName } = this.state
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (!_.isEqual(prevState.flyDataList, this.state.flyDataList)) {
      const newMapData = this.citizenName2MapData(this.state.flyDataList)
      this.setState({
        mapData: newMapData,
      })
  ...
  citizenName2MapData = (list) => {
    const tempMapData = this.state.mapData
    for (let index = 0; index < list.length; index++) {
      const { from, to } = list[index]

      //MapData(省会城市)中不存在的城市
      if (!tempMapData.find((g) => from.includes(g.city))) {
        const tempFrom = geoMap.find((g) => g.name.includes(from))
        if (tempFrom) {
          const parentId = tempFrom.parent
          const parent = geoMap.find((g) => g.treeID === parentId)
          tempMapData.push({ name: parent.name || '', city: from, latitude: tempFrom.center })
        }
      }

      if (!tempMapData.find((g) => to.includes(g.city))) {
        const tempTo = geoMap.find((g) => g.name.includes(to))
        if (tempTo) {
          const parentId = tempTo.parent
          const parent = geoMap.find((g) => g.treeID === parentId)
          tempMapData.push({ name: parent.name || '', city: to, latitude: tempTo.center })
        }
      }
    }
