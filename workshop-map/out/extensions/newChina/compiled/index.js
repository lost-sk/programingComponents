...
var CustomComp = /*#__PURE__*/function (_Component) {
  ...
  function CustomComp(props) {
          ...
          if (attr === 'dataList') {
            ...
            var finalList = mapData.map(function (m) {
              var findV = res.data.list.find(function (f) {
                return m.fullName.includes(f['name']);
              });
    ...
    _this2.state = {
      ...
      service: config.object != undefined ? config.object?.dynamicDataSource : {},
      backgroundColor: config.backgroundColor != undefined ? config.backgroundColor.color : 'rgba(13, 44, 105, 0.1)',
      zoom: config.zoom != undefined ? config.zoom.value : '1.5',
      center: config.center != undefined ? config.center.value : '107,30',
      scale: config.scale != undefined ? config.scale.value : 'enable',
      move: config.move != undefined ? config.move.value : 'enable',
      trigger: config.trigger != undefined ? config.trigger.value : 'mousemove',
      interval: config.interval != undefined ? config.interval.value : '0',
      topN: config.topN != undefined ? config.topN.value : '3',
    ...
    console.log('newchina config', config, _this2.state);
    return _this2;
  }
